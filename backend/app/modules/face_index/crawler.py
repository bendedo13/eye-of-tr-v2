...
async def run_crawl_job(job_id: int, db: Session):
    ...
    try:
        ...
        all_image_urls: List[tuple] = []  # (image_url, page_url)
        ...
        async with httpx.AsyncClient(timeout=15, follow_redirects=True, headers=headers) as client:
            ...
            async with httpx.AsyncClient(timeout=15, follow_redirects=True, headers=headers) as dl_client:
                for img_url, page_url in all_image_urls:
                    db.refresh(job)
                    if job.status == "cancelled":
                        return

                    # Check dedup by URL hash first
